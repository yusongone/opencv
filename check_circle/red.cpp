...
CvSize size320x240 = cvSize(320,240); // use a 320 x 240 size for all windows, also make sure your webcam is set to 320 x 240
    ...
    while(true){                                              // for each frame . . .
            ...
            p_seqCircles = cvHoughCircles(p_imgProcessed,           // input image, note that this has to be grayscale (no color)
                                                                      ...
                                                                      CV_HOUGH_GRADIENT,    // two pass algorithm for detecting circles, this is the only choice available for this parameter
            ...
            for(i=0; i < p_seqCircles->total; i++){         // for each element in sequential circles structure (i.e. for each object detected)
                    ...
                    printf("ball position (x,y) = (%f,%f), radius = %f \n", p_fltXYRadius[0],       // x position of center point of circle
                                                                                                                                    ...
                                                                                                                                    p_fltXYRadius[2]);      // z position of center point of circle
                    ...
                    cvCircle(p_imgOriginal,                                                                                                         // draw on original image
                                     cvPoint(cvRound(p_fltXYRadius[0]), cvRound(p_fltXYRadius[1])),         // center point of circle
                                     cvRound(p_fltXYRadius[2]),                                                                                     // radius of circle in pixels
                                     CV_RGB(250,0,0),                                                                                                       // draw pure red
